  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24 xl:py-32 bg-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        ...
        {/* Premium Services Grid - 2 Column Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12 sm:mb-16 lg:mb-20">
          {services.map((service, index) => (
            <motion.div
              key={service.id}
              className="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 lg:p-12 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden cursor-pointer"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="relative mb-6 lg:mb-8">
                  <div className="w-16 lg:w-20 h-16 lg:h-20 bg-gradient-to-br from-[#4A90E2] to-[#1E3A5F] rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-xl">
                    <Image
                      src={service.icon}
                      alt={`${service.title} icon`}
                      width={40}
                      height={40}
                      className="w-8 lg:w-10 h-8 lg:h-10 group-hover:scale-110 transition-transform duration-300"
                    />
                  </div>
                </div>

                {/* Service Content */}
                <h3 className="text-xl lg:text-2xl font-bold text-[#1E3A5F] mb-4 group-hover:text-[#4A90E2] transition-colors duration-300 leading-tight">
                  {service.title}
                </h3>

                <p className="text-gray-600 mb-6 leading-relaxed text-base lg:text-lg group-hover:text-gray-700 transition-colors duration-300">
                  {service.description}
                </p>
                ...
                <Link
                  href="/contact"
                  className="inline-flex items-center text-[#1E3A5F] font-bold hover:text-[#4A90E2] transition-all duration-300 text-sm lg:text-base group-hover:bg-gradient-to-r group-hover:from-[#4A90E2]/10 group-hover:to-[#2E7D32]/10 px-4 py-3 rounded-xl -mx-4"
                >
                  Learn More
